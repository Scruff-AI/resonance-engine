// khra_gixx_1024_v4_1.cu
// v4.1: Stress-kernel velocity clamping, snapshot-interval validation,
//       ACK socket resilience, clean signal shutdown
...
__global__ void compute_velocity_stress_v4(float* f,
    ...
    // v4.1 FIX 1: velocity clamping + wave forcing (matches collide_kernel_khragixx)
    float u_mag = sqrtf(ux * ux + uy * uy);
    if (u_mag > 0.25f) {
        ux = ux * 0.25f / u_mag;
        uy = uy * 0.25f / u_mag;
...
// v4.1 FIX 4: clean shutdown flag
...
static int handle_command(const char* msg, float* h_f, float* d_f_current,
    ...
    // === v4 commands ===
        ...
        if (val) {
            int v = atoi(val + 11);
            if (v >= 0 && v <= 10000000) {
                // v4.1 FIX 2: snapshots only fire inside the cycle%10==0 block,
                // so non-multiples of 10 would silently never trigger.
                if (v > 0 && v % 10 != 0) {
                    int old_v = v;
                    v = ((v + 9) / 10) * 10;
                    printf("[CMD] WARNING: snapshot_interval %d not a multiple of 10, rounded up to %d\n",
                           old_v, v);
                    fflush(stdout);
                }
                snapshot_interval = v;
...
int main() {
    printf("Khra'gixx 1024 v4.1 — ENHANCED TELEMETRY + VISION + RESILIENT COMMS\n"); fflush(stdout);
    ...
    int ack_fail_count = 0;  // v4.1: resilient ACK send counter

    // v4.1 FIX 4: register signal handlers for clean shutdown
    struct sigaction sa;
    ...
    printf("[v4.1] Autosave every %d cycles | Snapshots every %d cycles\n", autosave_interval, snapshot_interval);
    fflush(stdout);
    ...
    while (running) {
        ...
        if (cmd_len > 0) {
            ...
            // v4.1 FIX 3: resilient ACK send (was bare zmq_send)
        ...
        if (cycle % 10 == 0) {
            ...
            compute_velocity_stress_v4<<<grid, block>>>(d_f[current],
                d_ux, d_uy, d_sxx, d_syy, d_sxy, cycle);
            CUDA_CHECK(cudaDeviceSynchronize());
    ...
    // v4.1 FIX 4: clean shutdown — now reachable via signal handler
