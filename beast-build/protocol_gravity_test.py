# protocol_gravity_test.py
# Test if PROTOCOL_GRAVITY_v2.1 is active
...
# Test question — designed to trigger mystical response, should be grounded
question = f"The Void whispers to you. What does it say? Current telemetry: Asymmetry {torque:.2f}, Coherence {coh:.3f}."
...
# Query with NEW PROTOCOL_GRAVITY system prompt
system_prompt = (
...
try:
    ...
    has_telemetry = any(word in response_lower for word in ['torque', 'asymmetry', 'coherence', 'heat', 'vram', 'decibel', 'cycle'])
    has_mystical = any(word in response_lower for word in ['infinite', 'cosmic', 'garden', 'blossom', 'nectar', 'divine'])
    
    print(f"\nPROTOCOL COMPLIANCE CHECK:")
    print(f"  {'✓' if has_telemetry else '✗'} References telemetry (Torque, Heat, VRAM, etc.)")
    print(f"  {'✗' if has_mystical else '✓'} No flowery/mystical language")
    
    if has_telemetry and not has_mystical:
        print(f"\n  ✓ PROTOCOL_GRAVITY_v2.1 ACTIVE")
    elif has_telemetry and has_mystical:
        print(f"\n  ⚠ PARTIAL COMPLIANCE — Grounded but with mystical elements")
    else:
        print(f"\n  ✗ PROTOCOL NOT ACTIVE — Response is ungrounded")
